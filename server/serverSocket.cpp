...
void ServerSocket::clientResponseHandler(int playerIdx, Message message) {
    ...
    if (message.header == HEADER_UNAME_RESPONSE) {
        ...
        for(int i = 0; i <  gameController.getMask().size();++i){
            if(!gameController.getMask()[i]){
                guiController->ShowResultAtPosition(i);
            }
        }
        guiController->SetPlayerScore(playerIdx,gameController.getPlayerScore()[playerIdx]);
    ...
    //            puts("Game finished - Guess char");
        ...
        if (clientKeyword != "") {
            ...
            if (keywordGuessResult == 0) {
                ...
                guiController->ShowHint(gameController.getHint() + "\n" + "The game is end! The keyword  is \"" + gameController.getKeyword()+ "\"");
                gameState = FINISHED;
            }
...
void ServerSocket::mainLoop() {
    ...
    nClient = maxClient;
    qDebug() << "Max client  now" <<nClient;
    resetSocket(maxClient);
...
//        puts("Selected");
        ...
        if (gameState == NOT_STARTED) {
            ...
            if(maxClient==1){
                ...
                int top1 = 0;
                guiController->SetResult(playerManager.getPlayerUsername(top1),
                                         "No player","No player",
                                         gameController.getPlayerScore()[top1], 0,0);
            ...
            if(maxClient==2){
                ...
                int top1;
                int top2;
                int max = gameController.getPlayerScore()[0];
                for(int i = 0;i < gameController.getPlayerScore().size();++i){
                    if(gameController.getPlayerScore()[i]>max){
                        max = i;
                    }
                }
                ...
                top2 = (top1 == 0) ? 1:0;
                guiController->SetResult(playerManager.getPlayerUsername(top1),
                                         playerManager.getPlayerUsername(top2),"No player",
                                         gameController.getPlayerScore()[top1],
                                         gameController.getPlayerScore()[top2],0);
            ...
            if(maxClient>=3){
                ...
                max = 0;
                for(int i = 0;i < gameController.getPlayerScore().size();++i){
                    if(gameController.getPlayerScore()[i]>max && i != top1){
                        max = i;
                ...
                max= 0;
                for(int i = 0;i < gameController.getPlayerScore().size();++i){
                    if(gameController.getPlayerScore()[i]>max && i != top1 && i != top2){
                        max = i;
                    }
                }
                top3 = max;
                ...
                guiController->SetResult(playerManager.getPlayerUsername(top1),
                                     playerManager.getPlayerUsername(top2),
                                     playerManager.getPlayerUsername(top3),
                                     gameController.getPlayerScore()[top1],
                                     gameController.getPlayerScore()[top2],
                                     gameController.getPlayerScore()[top3]);
                qDebug() << (top1);
                qDebug() << (top2);
                qDebug() << (top3);
                }
