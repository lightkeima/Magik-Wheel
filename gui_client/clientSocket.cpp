...
Message ClientSocket::serverResponseHandler(Message message) {
    ...
    if (message.header == HEADER_UNAME_REQUEST || message.header == HEADER_UNAME_INVALID || message.header == HEADER_UNAME_DUPLICATED) {
        ...
        guiController->SetVisibility("waitPanel",false);

        cout << "Enter your username (not exceeding 10 characters, consist of letters, numbers and '_'): ";
        //cin >> username;
        while(guiController->GetTextValue("tfusername","text") == ""  || !guiController->GetAcceptedState("buttonAccept","accepted")){
        }
        username = guiController->GetTextValue("tfusername","text");
        ...
        guiController->SetWaitPanelText("Game has already started. Please wait for another game.");
    }
        ...
        guiController->SetTurnBool(true);
        char guessChar;
        //cin >> guessChar;
        string a = "N";
        while(a == "N"){
            a = guiController->GetCharGuess();
        }
        guiController->SetTurnBool(false);
        guessChar = a[0];
        ...
        guiController->SetVisibility("guesspanel",true);
        string response;
        //cin >> response;
        while(guiController->GetAcceptedState("guesspanel","visible")){
        }
        if(guiController->GetAcceptedState("buttonGuessAccept","accepted")){

            if(guiController->GetTextValue("tfguess","text").length()!=0){
                string keyword;
                keyword = guiController->GetTextValue("tfguess","text");
                data = {keyword};
            }
        }
        ...
        if (result == 1) {
            ...
            guiController->ShowHint("Congratulation! Your keyword is correct!\n");
        }
        else {
            printf("Your keyword is incorrect! Better luck next time...\n");
            guiController->ShowHint("Your keyword is incorrect! Better luck next time...\n");
        ...
        printf("Keyword: %s\n", maskedKeyword.c_str());
        for(int i = 0;i < maskedKeyword.length();++i){
            if(maskedKeyword[i] != '*'){
                guiController->ShowResultAtPosition(i);
            }
        }
        ...
        if(usernames.size()==1){
            ...
            int top1 = 0;
            guiController->SetResult(usernames[top1],
                                     "No player","No player",
                                     score[top1], 0,0);
